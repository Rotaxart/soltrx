...
import { FARM_STATE_LAYOUT_V6, FARM_VERSION_TO_LEDGER_LAYOUT, FARM_VERSION_TO_STATE_LAYOUT, } from "./layout";
...
export class Farm extends Base {
    ...
    static async makeCreateFarmInstructionV6({ connection, userKeys, poolInfo }) {
        ...
        frontInstructions.push(SystemProgram.createAccount({
    ...
    /* ================= fetch data ================= */
    static async fetchMultipleInfoAndUpdate({ connection, pools, owner, config }) {
    ...
    static async makeCreatorWithdrawFarmRewardInstructionV6Simple({ connection, poolKeys, userKeys, withdrawMint, }) {
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
        const endInstructionsType = [];
    ...
    static async makeCreateFarmInstructionV6Simple({ connection, userKeys, poolInfo }) {
        const farmId = Keypair.generate();
        const lamports = await connection.getMinimumBalanceForRentExemption(FARM_STATE_LAYOUT_V6.span);
        const frontInstructions = [];
        const endInstructions = [];
    ...
    static async makeRestartFarmInstructionV6Simple({ connection, poolKeys, userKeys, newRewardInfo, }) {
    ...
    static async makeFarmCreatorAddRewardTokenInstructionV6Simple({ connection, poolKeys, userKeys, newRewardInfo, }) {
    ...
    static async makeDepositInstructionSimple({ connection, poolKeys, fetchPoolInfo, ownerInfo, amount, associatedOnly = true }) {
    ...
    static async makeWithdrawInstructionSimple({ connection, fetchPoolInfo, ownerInfo, amount, associatedOnly = true }) {
        const ownerMintToAccount = {};
        for (const item of ownerInfo.tokenAccounts) {
            if (associatedOnly) {
                const ata = getATAAddress(ownerInfo.wallet, item.accountInfo.mint).publicKey;
