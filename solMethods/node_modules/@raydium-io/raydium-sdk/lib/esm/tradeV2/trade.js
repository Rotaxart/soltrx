...
import { findProgramAddress, forecastTransactionSize, jsonInfo2PoolKeys, parseSimulateLogToJson, parseSimulateValue, simulateMultipleInstruction, } from "../common";
import { ONE, Percent, Price, Token, TokenAmount, ZERO } from "../entity";
...
export class TradeV2 extends Base {
            ...
            for (const iFromPool of info.in) {
                ...
                for (const iOutPool of info.out) {
                    ...
                    try {
                        ...
                        const infoAPoolOpen = iFromPool.version === 6 ? true : simulateCache[iFromPool.id].startTime.toNumber() < chainTime;
                        const infoBPoolOpen = iOutPool.version === 6 ? true : simulateCache[iOutPool.id].startTime.toNumber() < chainTime;
    ...
    static computeAmountOut({ middleMintInfo, amountIn, currencyOut, slippage, fromPool, toPool, simulateCache, tickCache, }) {
        ...
        if (fromPool.version === 6) {
            const { minAmountOut: _minMiddleAmountOut, priceImpact: _firstPriceImpact, fee: _firstFee, remainingAccounts: _firstRemainingAccounts } = AmmV3.computeAmountOutFormat({
                poolInfo: fromPool,
        ...
        if (toPool.version === 6) {
            const { amountOut: _amountOut, minAmountOut: _minAmountOut, priceImpact: _secondPriceImpact, fee: _secondFee, remainingAccounts: _secondRemainingAccounts } = AmmV3.computeAmountOutFormat({
                poolInfo: toPool,
    ...
    static makeSwapInstruction({ routeProgram, ownerInfo, inputMint, swapInfo }) {
        if (swapInfo.routeType === 'amm') {
            ...
            return {
                ...
                innerTransaction: {
                    instructions: [
                        route1Instruction(routeProgram, poolKey1, poolKey2, ownerInfo.sourceToken, ownerInfo.routeToken, ownerInfo.userPdaAccount, ownerInfo.wallet, inputMint, swapInfo.amountIn.raw, swapInfo.minAmountOut.raw, swapInfo.remainingAccounts[0]),
                        route2Instruction(routeProgram, poolKey1, poolKey2, ownerInfo.routeToken, ownerInfo.destinationToken, ownerInfo.userPdaAccount, ownerInfo.wallet, swapInfo.middleMint, swapInfo.remainingAccounts[1])
                    ],
                    signers: [],
                    lookupTableAddress: [],
    ...
    static getAssociatedMiddleStatusAccount({ programId, fromPoolId, middleMint, owner, }) {
        const { publicKey } = findProgramAddress([fromPoolId.toBuffer(), middleMint.toBuffer(), owner.toBuffer()], programId);
        return publicKey;
    }
}
