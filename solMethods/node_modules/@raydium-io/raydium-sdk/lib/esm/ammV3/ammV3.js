...
import { closePositionInstruction, collectRewardInstruction, createPoolInstruction, decreaseLiquidityInstruction, increaseLiquidityInstruction, initRewardInstruction, openPositionInstruction, setRewardInstruction, swapInstruction, } from "./instrument";
...
import { getPdaMetadataKey, getPdaOperationAccount, getPdaPersonalPositionAddress, getPdaPoolId, getPdaPoolRewardVaulId, getPdaPoolVaultId, getPdaProtocolPositionAddress, getPdaTickArrayAddress, } from "./utils/pda";
...
export class AmmV3 extends Base {
    static makeMockPoolInfo({ programId, mint1, mint2, ammConfig, createPoolInstructionSimpleAddress, initialPrice, owner }) {
    ...
    // transaction
    static async makeCreatePoolInstructionSimple({ connection, programId, owner, mint1, mint2, ammConfig, initialPrice, computeBudgetConfig }) {
    ...
    static async makeOpenPositionInstructionSimple({ connection, poolInfo, ownerInfo, tickLower, tickUpper, liquidity, slippage, associatedOnly = true, computeBudgetConfig }) {
        const { instructions, instructionTypes } = computeBudgetConfig ? addComputeBudget(computeBudgetConfig).innerTransaction : { instructions: [], instructionTypes: [] };
        ...
        //   poolInfo.mintB.decimals
        // );
        const { amountSlippageA, amountSlippageB } = LiquidityMath.getAmountsFromLiquidityWithSlippage(poolInfo.sqrtPriceX64, SqrtPriceMath.getSqrtPriceX64FromTick(tickLower), SqrtPriceMath.getSqrtPriceX64FromTick(tickUpper), liquidity, true, true, slippage);
    ...
    static async makeIncreaseLiquidityInstructionSimple({ connection, poolInfo, ownerPosition, ownerInfo, liquidity, slippage, associatedOnly = true, computeBudgetConfig }) {
        const { instructions, instructionTypes } = computeBudgetConfig ? addComputeBudget(computeBudgetConfig).innerTransaction : { instructions: [], instructionTypes: [] };
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
        const endInstructionsType = [];
        const signers = [];
        const { amountSlippageA, amountSlippageB } = LiquidityMath.getAmountsFromLiquidityWithSlippage(poolInfo.sqrtPriceX64, SqrtPriceMath.getSqrtPriceX64FromTick(ownerPosition.tickLower), SqrtPriceMath.getSqrtPriceX64FromTick(ownerPosition.tickUpper), liquidity, true, true, slippage);
        const mintAUseSOLBalance = ownerInfo.useSOLBalance && poolInfo.mintA.mint.equals(Token.WSOL.mint);
        const mintBUseSOLBalance = ownerInfo.useSOLBalance && poolInfo.mintB.mint.equals(Token.WSOL.mint);
        const ownerTokenAccountA = await this._selectOrCreateTokenAccount({
            mint: poolInfo.mintA.mint,
    ...
    static async makeDecreaseLiquidityInstructionSimple({ connection, poolInfo, ownerPosition, ownerInfo, liquidity, amountMinA, amountMinB, slippage, associatedOnly = true, computeBudgetConfig }) {
        const { instructions, instructionTypes } = computeBudgetConfig ? addComputeBudget(computeBudgetConfig).innerTransaction : { instructions: [], instructionTypes: [] };
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
        ...
        if (amountMinA !== undefined && amountMinB !== undefined) {
            ...
            const { amountSlippageA, amountSlippageB } = LiquidityMath.getAmountsFromLiquidityWithSlippage(poolInfo.sqrtPriceX64, SqrtPriceMath.getSqrtPriceX64FromTick(ownerPosition.tickLower), SqrtPriceMath.getSqrtPriceX64FromTick(ownerPosition.tickUpper), liquidity, false, true, slippage ?? 0);
    ...
    static async makeSwapBaseInInstructionSimple({ connection, poolInfo, ownerInfo, inputMint, amountIn, amountOutMin, priceLimit, remainingAccounts, associatedOnly = true, computeBudgetConfig }) {
        const { instructions, instructionTypes } = computeBudgetConfig ? addComputeBudget(computeBudgetConfig).innerTransaction : { instructions: [], instructionTypes: [] };
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
    ...
    static async makeSwapBaseOutInstructionSimple({ connection, poolInfo, ownerInfo, outputMint, amountOut, amountInMax, priceLimit, remainingAccounts, associatedOnly = true, computeBudgetConfig }) {
        const { instructions, instructionTypes } = computeBudgetConfig ? addComputeBudget(computeBudgetConfig).innerTransaction : { instructions: [], instructionTypes: [] };
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
    ...
    static async makeInitRewardInstructionSimple({ connection, poolInfo, ownerInfo, rewardInfo, chainTime, associatedOnly = true }) {
    ...
    static async makeInitRewardsInstructionSimple({ connection, poolInfo, ownerInfo, rewardInfos, associatedOnly = true, computeBudgetConfig }) {
    ...
    static async makeSetRewardInstructionSimple({ connection, poolInfo, ownerInfo, rewardInfo, chainTime, associatedOnly = true, }) {
        logger.assertArgument(rewardInfo.endTime > rewardInfo.openTime, "reward time error", "rewardInfo", rewardInfo);
        logger.assertArgument(rewardInfo.openTime > chainTime, "reward must be paid later", "rewardInfo", rewardInfo);
        const frontInstructions = [];
        const endInstructions = [];
    ...
    static async makeSetRewardsInstructionSimple({ connection, poolInfo, ownerInfo, rewardInfos, chainTime, associatedOnly = true, computeBudgetConfig }) {
        const { instructions, instructionTypes } = computeBudgetConfig ? addComputeBudget(computeBudgetConfig).innerTransaction : { instructions: [], instructionTypes: [] };
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
    ...
    static async makeCollectRewardInstructionSimple({ connection, poolInfo, ownerInfo, rewardMint, associatedOnly = true, computeBudgetConfig }) {
        const { instructions, instructionTypes } = computeBudgetConfig ? addComputeBudget(computeBudgetConfig).innerTransaction : { instructions: [], instructionTypes: [] };
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
    ...
    static async makeCollectRewardsInstructionSimple({ connection, poolInfo, ownerInfo, rewardMints, associatedOnly = true, computeBudgetConfig }) {
        const { instructions, instructionTypes } = computeBudgetConfig ? addComputeBudget(computeBudgetConfig).innerTransaction : { instructions: [], instructionTypes: [] };
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
    ...
    static async makeHarvestAllRewardInstructionSimple({ connection, fetchPoolInfos, ownerInfo, associatedOnly = true }) {
    ...
    // instrument
    static async makeCreatePoolInstructions({ connection, programId, owner, mintA, mintB, ammConfigId, initialPriceX64 }) {
        ...
        const instructions = [
            ...
            createPoolInstruction(programId, poolId, owner, ammConfigId, observationId.publicKey, mintA.mint, mintAVault, mintB.mint, mintBVault, initialPriceX64)
        ];
        return {
            address: {
    ...
    static makeIncreaseLiquidityInstructions({ poolInfo, ownerPosition, ownerInfo, liquidity, amountSlippageA, amountSlippageB }) {
        ...
        return {
            ...
            innerTransaction: {
                instructions: [
                    increaseLiquidityInstruction(poolInfo.programId, ownerInfo.wallet, positionNftAccount, personalPosition, poolInfo.id, protocolPosition, tickArrayLower, tickArrayUpper, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, poolInfo.mintA.vault, poolInfo.mintB.vault, liquidity, amountSlippageA, amountSlippageB)
                ],
                signers: [],
    ...
    static makeDecreaseLiquidityInstructions({ poolInfo, ownerPosition, ownerInfo, liquidity, amountMinA, amountMinB }) {
        ...
        return {
            ...
            innerTransaction: {
                instructions: [
                    decreaseLiquidityInstruction(poolInfo.programId, ownerInfo.wallet, positionNftAccount, personalPosition, poolInfo.id, protocolPosition, tickArrayLower, tickArrayUpper, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, poolInfo.mintA.vault, poolInfo.mintB.vault, rewardAccounts, liquidity, amountMinA, amountMinB)
                ],
                signers: [],
    ...
    static makeInitRewardInstructions({ poolInfo, ownerInfo, rewardInfo }) {
        ...
        return {
            ...
            innerTransaction: {
                instructions: [
                    initRewardInstruction(poolInfo.programId, ownerInfo.wallet, poolInfo.id, operationId, poolInfo.ammConfig.id, ownerInfo.tokenAccount, rewardInfo.mint, poolRewardVault, rewardInfo.openTime, rewardInfo.endTime, rewardInfo.emissionsPerSecondX64),
                ],
                signers: [],
    ...
    static makeSetRewardInstructions({ poolInfo, ownerInfo, rewardInfo }) {
        ...
        return {
            ...
            innerTransaction: {
                instructions: [
                    setRewardInstruction(poolInfo.programId, ownerInfo.wallet, poolInfo.id, operationId, poolInfo.ammConfig.id, ownerInfo.tokenAccount, rewardVault, rewardIndex, rewardInfo.openTime, rewardInfo.endTime, rewardInfo.emissionsPerSecondX64)
                ],
                signers: [],
                instructionTypes: [InstructionType.clmmInitReward],
                supportedVersion: [TxVersion.LEGACY, TxVersion.V0]
    ...
    static makeCollectRewardInstructions({ poolInfo, ownerInfo, rewardMint }) {
        ...
        return {
            ...
            innerTransaction: {
                instructions: [
                    collectRewardInstruction(poolInfo.programId, ownerInfo.wallet, poolInfo.id, ownerInfo.tokenAccount, rewardVault, rewardIndex)
                ],
                signers: [],
                instructionTypes: [InstructionType.clmmInitReward],
                supportedVersion: [TxVersion.LEGACY, TxVersion.V0]
    ...
    static computeAmountOutFormat({ poolInfo, tickArrayCache, amountIn, currencyOut, slippage }) {
        ...
        const _currentPrice = new Price(amountInIsTokenAmount ? amountIn.token : amountIn.currency, new BN(10).pow(new BN(20 + (amountInIsTokenAmount ? amountIn.token : amountIn.currency).decimals)), currencyOut instanceof Token ? currencyOut : Token.WSOL, currentPrice.mul(new Decimal(10 ** (20 + (currencyOut instanceof Token ? currencyOut : Token.WSOL).decimals))).toFixed(0));
        const _executionPrice = new Price(amountInIsTokenAmount ? amountIn.token : amountIn.currency, new BN(10).pow(new BN(20 + (amountInIsTokenAmount ? amountIn.token : amountIn.currency).decimals)), currencyOut instanceof Token ? currencyOut : Token.WSOL, executionPrice.mul(new Decimal(10 ** (20 + (currencyOut instanceof Token ? currencyOut : Token.WSOL).decimals))).toFixed(0));
    ...
    static computeAmountOut({ poolInfo, tickArrayCache, baseMint, amountIn, slippage, priceLimit = new Decimal(0) }) {
        let sqrtPriceLimitX64;
        ...
        const { expectedAmountOut, remainingAccounts, executionPrice: _executionPriceX64, feeAmount } = PoolUtils.getOutputAmountAndRemainAccounts(poolInfo, tickArrayCache, baseMint, amountIn, sqrtPriceLimitX64);
    ...
    static computeAmountIn({ poolInfo, tickArrayCache, baseMint, amountOut, slippage, priceLimit = new Decimal(0) }) {
        let sqrtPriceLimitX64;
        if (priceLimit.equals(new Decimal(0))) {
        ...
        const { expectedAmountIn, remainingAccounts, executionPrice: _executionPriceX64, feeAmount } = PoolUtils.getInputAmountAndRemainAccounts(poolInfo, tickArrayCache, baseMint, amountOut, sqrtPriceLimitX64);
        const _executionPrice = SqrtPriceMath.sqrtPriceX64ToPrice(_executionPriceX64, poolInfo.mintA.decimals, poolInfo.mintB.decimals);
        const executionPrice = baseMint.equals(poolInfo.mintA.mint) ? _executionPrice : new Decimal(1).div(_executionPrice);
    ...
    // fetch data
    static async fetchMultiplePoolInfos({ connection, poolKeys, ownerInfo, chainTime, batchRequest = false }) {
