...
class MathUtil {
    ...
    static x64ToDecimal(num, decimalPlaces) {
        return new decimal_js_1.default(num.toString())
            .div(decimal_js_1.default.pow(2, 64))
...
class SqrtPriceMath {
    ...
    static getSqrtPriceX64FromTick(tick) {
        ...
        let ratio = (tickAbs & 0x1) != 0
            ? new bn_js_1.default("18445821805675395072")
            : new bn_js_1.default("18446744073709551616");
        if ((tickAbs & 0x2) != 0)
    ...
    static getTickFromSqrtPriceX64(sqrtPriceX64) {
        if (sqrtPriceX64.gt(constants_1.MAX_SQRT_PRICE_X64) ||
            sqrtPriceX64.lt(constants_1.MIN_SQRT_PRICE_X64)) {
...
class SwapMath {
    static swapCompute(programId, poolId, tickArrayCache, zeroForOne, fee, liquidity, currentTick, tickSpacing, currentSqrtPriceX64, amountSpecified, lastSavedTickArrayStartIndex, sqrtPriceLimitX64) {
        ...
        while (!state.amountSpecifiedRemaining.eq(entity_1.ZERO) &&
            ...
            state.tick < constants_1.MAX_TICK &&
            state.tick > constants_1.MIN_TICK) {
            if (loopCount > 10) {
                throw Error("liquidity limit");
            }
            const step = {};
            step.sqrtPriceStartX64 = state.sqrtPriceX64;
            const { nextTick: nextInitTick, tickArrayAddress, tickArrayStartTickIndex: tickAarrayStartIndex } = tickQuery_1.TickQuery.nextInitializedTick(programId, poolId, tickArrayCache, state.tick, tickSpacing, zeroForOne);
            ...
            [state.sqrtPriceX64, step.amountIn, step.amountOut, step.feeAmount] =
                SwapMath.swapStepCompute(state.sqrtPriceX64, targetPrice, state.liquidity, state.amountSpecifiedRemaining, fee);
    ...
    static swapStepCompute(sqrtPriceX64Current, sqrtPriceX64Target, liquidity, amountRemaining, feeRate) {
        ...
        if (!baseInput &&
            swapStep.amountOut.gt(amountRemaining.mul(constants_1.NEGATIVE_ONE))) {
